  ...
  (error) => {
    ...
    */
    ...
    if (statusValue === 401 && ok === 1) {
      ...
      navigator("/");
    } else if (statusValue === 401 && ok === 3) {
      alert("로그인 후 이용 가능한 기능입니다");
      removeCookie("token", { path: "/" });
      navigator("/");
    } else if (statusValue === 401 && ok === 4) {
      alert("비정상적인 활동이 감지되어 로그아웃됩니다.");
      removeCookie("token", { path: "/" });
      navigator("/");
    } else if (statusValue !== 401) {
      alert("비정상적인 활동이 감지되어 로그아웃됩니다.");
      removeCookie("token", { path: "/" });
      navigator("/");
    }
...
instanceF.interceptors.request.use(
  async (config) => {
    ...
    // console.log(token);
    // 요청 성공 직전 수행할 일
    console.log("인터셉터정보 161줄", config); //여기부터 요청시작
    console.log(config.headers);
    console.log(config.headers.Authorization); //이건 Bearer만 찍히는 게 맞아.

    config.headers["Authorization"] = `Bearer ${token}`;
    console.log("토큰 여기까지 찍혀 166줄", token);
    // console.log("accTokenPm1155", token);
